...
internal class ProtoManifest
{
    ...
    [ProtoMember(1)] public List<FileData> Files { get; private set; }

    [ProtoMember(2)] public ulong Id { get; private set; }

    [ProtoMember(3)] public DateTime CreationTime { get; private set; }
    ...
    public DepotManifest ConvertToSteamManifest(uint depotId)
    {
        ...
        {
            Files = new List<DepotManifest.FileData>(Files.Count)
        };

        foreach (var file in Files)
        {
            var fileNameHash =
                SHA1.HashData(Encoding.UTF8.GetBytes(file.FileName.Replace('/', '\\').ToLowerInvariant()));
            var chunkCount = file.Chunks?.Count ?? 0;
            ...
            if (file.Chunks != null)
                foreach (var chunk in file.Chunks)
                {
                    var newChunk = new DepotManifest.ChunkData(chunk.ChunkId, BitConverter.ToUInt32(chunk.Checksum, 0),
                        chunk.Offset, chunk.CompressedLength, chunk.UncompressedLength);
    ...
    public class FileData
    {
        ...
        [ProtoMember(1)] public string FileName { get; internal set; }
    ...
    public class ChunkData(DepotManifest.ChunkData sourceChunk)
    {
        ...
        /// </summary>
        [ProtoMember(2)]
        public byte[] Checksum { get; private set; } = BitConverter.GetBytes(sourceChunk.Checksum);
